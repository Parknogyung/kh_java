...
public class A_Date {
	public void method1() {
		...
		Date now = new Date(); // java.utll로 해주기
		...
		when = new Date(1000L); // 밀리세컨드 단위이기 때문에 1000으로 줘야 1초가 지나감
		
		System.out.println(when);
		
		// Deprecated 된 생성자를 사용한 Date 객체 생성
		when = new Date((2022 - 1900), (4-1), 22); // 년도에서 1900을 빼주고 월은 0부터 시작이라 이런식으로 해줘야함
