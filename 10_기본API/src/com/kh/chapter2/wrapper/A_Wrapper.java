...
public class A_Wrapper {
	public void method1() {
		...
		// 클래스에서 제공하는 메소드들을 들을 수 있다.
		System.out.println(double1.equals(double2));
		// 두 값을 비교해서 같으면 0, double의 값이 크면 1, 작으면 -1을 리턴한다.
		...
		// 3. Auto Boxing 통한 방법
		Integer integer2 = 5; // 자동으로 Integer 객체로 만들어줌. 같은 타입만 Auto Boxing이 가능하다. ex) 5.5, false
		Double double3 = 3.555; // 3.555f하면 에러남. 무조건 같은 타입으로만 해줘야한다.
		...
		int iNum2 = integer.intValue(); // int로 반환해준다.
	...
	public void method2() {
		...
		System.out.println(Integer.valueOf(iNum));
		System.out.println(Double.valueOf(dNum));
	}
}