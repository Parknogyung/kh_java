...
public class Application {
	...
	public static void main(String[] args) {
		 ...
		 * 		  - a -> System.out.println(a);  // 매개변수가 하나일 때, 실행문이 하나일 때 소괄호(), 중괄호() 를 생략 가능
...
//		new A_Lambda().method1();
		 ...
		 *  - 구현할 추상메소드에 따라서 return, 매개변수 등 넣고 안넣고를 정한다.
		 * 
		 * 함수적 인터페이스 (Functional Interface)
		 * 	- 모든 인터페이스가 타겟 타입으로 사용할 수 없다.
		 *  - 하나의 추상 메소드가 선언된 인터페이스만 람다식의 타겟이 될 수 있는데 이러한 인터페이스를 함수적 인터페이스라고 한다.
		 *  - 함수적 인터페이스를 선언할 때 @FunctionalInterface 어노테이션을 붙이면 두 개 이상의 추상메소드가 선언되지 않도록 
		 *    컴파일러가 체크해 준다.
		 */
...
//		new E_Predicate().method1();
		 ...
		 * 	 - 메소드 참조는 참조해서 매개 변수의 정보 및 리턴 타입을 알아내어, 람다식에서 불필요한 매개 변수를 제거하는 것이 목적이다.
